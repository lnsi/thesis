...
namespace VncSharp.Encodings
{
	...
	public abstract class EncodedRectangle : IDesktopUpdater
	{
		...
		public unsafe virtual void Draw(Bitmap desktop)
		{
			...
			BitmapData bmpd = desktop.LockBits(new Rectangle(new System.Drawing.Point(0,0), desktop.Size), ImageLockMode.ReadWrite, desktop.PixelFormat);
    ...
    /// </summary>
    /// <param name="desktop">The image the represents the remote desktop. NOTE: this image will be altered.</param>
    public unsafe virtual void Draw(WriteableBitmap desktop)
    {
      try
      {
        desktop.Lock();
        Int32Rect _rect = new Int32Rect(rectangle.X, rectangle.Y, rectangle.Width, rectangle.Height);
        int _bytesPerPixel = framebuffer.BitsPerPixel / 8;
        int _stride = rectangle.Width * _bytesPerPixel;
        desktop.WritePixels(_rect, framebuffer.ByteArray, _stride, 0);
      }
      catch (Exception e)
      {
      }
