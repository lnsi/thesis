...
To reach this goal, the design principle of conceptual consistency was used, as defined by \cite{Benyon:2010}.
...
\begin{figure}[htb]
  \centering
    \includegraphics[width=0.8\textwidth]{images/scenarios}
...
The tangible UI involve using the smartphone as a controlling device for specific application features.
It should fulfill the following requirements:
...
Lastly, it was decided that the smartphone would be used as a tangible UI only to trigger the pairing procedure.
This decision was based on a careful analysis of the context of use and oral feedback gathered from the prototyping sessions, that showed that users might be reticent to leave their smartphones on the table for fear of being robbed, or to drag the device across the table for fear of damaging it.
\\
\linebreak
The only input technique considered in the design of the surface UI is touch-based interaction.
The reason for this is that it is strongly consistent with both the smartphone experience and the tabletop experience, that both rely on touch-based interaction.
...
\item Closing the replicated UI can be done by: (1) closing the minimized surface UI, (2) dragging the surface UI to a corner of the tabletop or (3) pressing and holding a finger on the active border.
...
As mentioned in section~\ref{sec:interaction}, the use of the smartphone as a tangible UI is limited to triggering the pairing procedure.
